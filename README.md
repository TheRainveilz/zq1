...
| 项目 | 结果 |
|---|---|
| 真实样本（正面 1 张） | 10 个兜孔中 9 个双特征全过，判定 OK；2 孔模式 154 ms / 全 10 孔 267 ms |
| 特异性对照 | 在 47 个"确定无压痕"位置跑压痕检测器，**0 误命中** |
| 合成样本 | 正/反面 × (基准、旋转+偏移、强噪声) + 空白无工件，**7/7 判定正确**，反面走 `NG_NO_FEATURE` |

## 已知限制

1. **仅有 1 张真实样本**（正面），反面阈值靠对照窗口与合成图推定，**上线前必须用真实反面样本复核**
   `MIN_VALID_MARKS` 与 `MARK_CIRCULARITY_MIN`。建议补 ≥20 张反面 + ≥10 张边界样本
   （漏冲压痕、油污遮挡、严重毛刺）。
2. **"外圆定位"改为节圆定位**：样本中垫片外圆在视场外（工件仅上半幅进入画面），
   照字面找外圆会失败。外圆 Hough 仍保留为二级兜底。
3. **轮廓计数（特征A）鉴别力弱**：反面单圈冲裁边的内、外沿也会被计成 2 圈
   （合成反面测试中 14/18 个孔误通过特征A）。真正的鉴别力来自特征B（拐角压痕）。
   因此 **`HOLE_LOGIC = "AND"` 不可改为 `OR`**，否则反面会漏判。
...
| 正面误判 NG，框位置对但圆度低 | `MARK_HOUGH_P2` 调小、`MIN_VALID_MARKS` 降到 1、放宽 `MARK_R_RATIO_RANGE` |
| 反面误判 OK | `FEATURE_A_MODE="contour_and_hough"`、`MIN_VALID_MARKS` 提到 3、`MARK_CIRCULARITY_MIN` 提到 0.80 |
